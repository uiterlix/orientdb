...
public class ORecordSerializerSchemaAware2CSV extends ORecordSerializerCSVAbstract {
	...
	@Override
	protected StringBuilder toString(ORecordInternal<?> iRecord, final StringBuilder iOutput, final String iFormat, OUserObject2RecordHandler iObjHandler,
			final Set<ODocument> iMarshalledRecords, final boolean iOnlyDelta, final boolean autoDetectCollectionType) {
		if (iRecord == null)
		...
		for (String fieldName : fieldNames) {
			...
			if (prop != null && prop.getType() != OType.ANY) {
				...
				if (type == null) {
					...
					else if (ODatabaseRecordThreadLocal.INSTANCE.isDefined() && ODatabaseRecordThreadLocal.INSTANCE.get().getDatabaseOwner() instanceof ODatabaseObject
							&& ((ODatabaseObject) ODatabaseRecordThreadLocal.INSTANCE.get().getDatabaseOwner()).getEntityManager().getEntityClass(fieldClassName) != null) {
						// DETERMINE THE FIELD TYPE
						type = OType.LINK;
						linkedClass = getLinkInfo(ODatabaseRecordThreadLocal.INSTANCE.get(), fieldClassName);
	...
	public ORecordInternal<?> fromString(String iContent, final ORecordInternal<?> iRecord, final String[] iFields) {
		return new ORecordStringUnmarshaller().parse(iContent, iRecord, iFields);
	}
