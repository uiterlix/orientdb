...
public class ORecordStringUnmarshaller {
  ...
  private Object parseBinary(char[] chars, ORecord<?> iSourceRecord, OType type, OType linkedType) {
    ...
    return OStringSerializerHelper.getBinaryContent(builder.toString());
  }
  ...
  private Object parseList(char[] chars, ORecord<?> iSourceRecord, OType type, OType linkedtType) {
    if (type == OType.LINKLIST) {
      ...
      OTrackedList<Object> result = new OTrackedList<Object>(iSourceRecord);
      i++;
      if (i < chars.length && chars[i] == OStringSerializerHelper.LIST_END) {
        i++;
        return result;
  ...
  private Object parseSet(char[] chars, ORecord<?> iSourceRecord, OType type, OType linkedtType) {
    if (type == OType.LINKSET) {
      ...
      OMVRBTreeRIDSet set = new OMVRBTreeRIDSet(iSourceRecord);
      set.setAutoConvertToRecord(false);
      set.fromStream(builder);
      return set;

    } else {
      OTrackedSet<Object> result = new OTrackedSet<Object>(iSourceRecord);
      i++;
      if (i < chars.length && chars[i] == OStringSerializerHelper.SET_END) {
        i++;
        return result;
  ...
  private Object parseMap(char[] chars, ORecord<?> iSourceRecord, OType type, OType iLinkedType) {
    ...
    if (result instanceof ORecordElement)
      ((ORecordElement) result).setInternalStatus(ORecordElement.STATUS.UNMARSHALLING);
    ...
    try {
      ...
      if (result instanceof ORecordElement)
        ((ORecordElement) result).setInternalStatus(ORecordElement.STATUS.LOADED);
    }
  ...
  private Object parseBag(char[] chars, ORecord<?> iSourceRecord) {
    i++;
    if (i < chars.length && chars[i] == OStringSerializerHelper.BAG_END) {
      i++;
      return null;
    }
    StringBuilder builder = new StringBuilder();
    while (i < chars.length && chars[i] != OStringSerializerHelper.BAG_END) {
      builder.append(chars[i]);
      i++;
    }
